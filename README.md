...
- `RobotTask` einführen.
- Roboter speichern aktive Task-Objekte statt nur Request-IDs.
- `TopAccessStrategy.next_action(state, task)` implementieren.
- Scheduler erzeugt keine vollständigen Pläne mehr.
- Nach jeder erfolgreichen Aktion wird die nächste Aktion geplant.

### 2. Realistische Aktionskosten

- Grid-Bewegung: 1 ZE pro horizontalem/vertikalem Nachbarschritt.
- Armbewegung: Kosten abhängig von Zugriffstiefe.
- Pickstation-Servicezeit: 4–6 ZE.
- Roboter wird während der Pickstation-Bearbeitung wieder frei.
- Rücklagerung der blockierenden Bins und Target-Bin bleibt verpflichtend.

### 3. Einfache Relocation-Heuristik

- Funktion zur Auswahl eines geeigneten temporären Zielstacks.
- Anfangs einfache Heuristik, später optimierbar.
- Ziel: spätere Optimierungsverfahren nicht blockieren.

### 4. Hotness- und Nachfrageverteilungen

- Uniform-Verteilung.
- Zipf-Verteilung.
- ABC-Verteilung:
  - A: 20 % der Bins erzeugen 80 % der Requests.
  - B: 30 % der Bins erzeugen 15 % der Requests.
  - C: 50 % der Bins erzeugen 5 % der Requests.

### 5. Pickstations und Top-Down-Visualisierung

- Mehrere Pickstations außerhalb des Grids.
- Realistischeres Pathing zu Pickstations.
- Visualisierung von oben mit Grid, Robotern und Pickstations.
- Roboterfarben:
  - Grün: idle
  - Rot: Auftragsbearbeitung
  - Orange: trägt Target-Bin

### 6. Benchmarking

- Online-Planung: Entscheidungen während der Simulation.
- Offline-Planung: vollständiger Plan nach Initialisierung.
- Vergleich von Online-Plan gegen heuristisch/optimal geplanten Offline-Plan.
